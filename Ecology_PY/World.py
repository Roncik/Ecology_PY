...
class World(object):
	...
	def makeTurn(self, plaga):
		actions = []

		if plaga: #Mechanika Plagi
			self.Plague()
			print("W Tej Turze Panuję Plaga")

		self.organisms.sort(key=lambda o: o.initiative, reverse=False) #Dodane by Antylopa wykonywała ruch przed Rysiem
		...
		self.organisms.sort(key=lambda o: o.initiative, reverse=False)
		self.newOrganisms = []

		self.turn += 1

	def Plague(self): #Mechanika Plagi
		for org in self.organisms:
			org.liveLength *= 0.5
	...
	def makeMove(self, action):
		...
		elif action.action == ActionEnum.A_RUN: #Zachowanie Antylopy
			action.organism.position = action.position
